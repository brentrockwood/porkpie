...
describe("HeuristicTaskClassifier", () => {
  it("logs classification metadata without task content", async () => {
    ...
    await expect(classifier.classify({ title: "Review architecture", description: "Client notes", manualTags: [] })).resolves.toEqual([
      { name: "work", confidence: 0.85 },
    ]);

    expect(logger).toHaveBeenCalledWith({ classifier: "heuristic", outcome: "success", tagCount: 1 });
