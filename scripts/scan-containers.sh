...
docker run --rm -v /var/run/docker.sock:/var/run/docker.sock "$scanner_image" \
  image --scanners vuln --ignore-unfixed --severity HIGH,CRITICAL --exit-code 1 "$node_image" || status=1

docker run --rm -v /var/run/docker.sock:/var/run/docker.sock "$scanner_image" \
  image --scanners vuln --ignore-unfixed --severity HIGH,CRITICAL --exit-code 1 "$postgres_image" || status=1
